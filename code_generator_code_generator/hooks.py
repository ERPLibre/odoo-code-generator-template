...
When blacklist, all selected fields will be ignored.""",
    ...
    model_code_generator_add_model_wizard = code_generator_id.add_update_model(
        model_model,
        model_name,
        dct_field=dct_field,
    )
    ...
    model_code_generator_ir_model_fields = code_generator_id.add_update_model(
        model_model,
        model_name,
        dct_field=dct_field,
    )
...
action = {"type": "ir.actions.act_url", "target": "self", "url": "/code_generator/%s" % ','.join(records.mapped(lambda r: str(r.id)))}""",
    ...
    dct_field = {
        ...
        "target": {
            ...
            "required": True,
            "selection": ("[('new', 'New Window'), ('self', 'This Window')]"),
            "ttype": "selection",
        },
        "type": {
...
name, partner_id.name""",
        ...
        "is_hide_blacklist_graph_view": {
            ...
            "help": (
                "Hide from view when field is blacklisted. View graph" " only."
            ),
            "ttype": "boolean",
        },
        ...
        "is_hide_blacklist_pivot_view": {
            ...
            "help": (
                "Hide from view when field is blacklisted. View pivot" " only."
            ),
            "ttype": "boolean",
        },
        ...
        "track_visibility": {
            ...
            "selection": ("[('onchange', 'On Change'), ('always', 'Always')]"),
            "ttype": "selection",
        },
        "translate": {
...
* if False, the view currently does not extend its parent but can be enabled
     """,
        ...
        "is_code_generator": {
            "field_description": "Is Code Generator",
            "help": (
                "Do a link with code generator to show associate" " ir.ui.view"
            ),
            "ttype": "boolean",
        },
        "is_hide_blacklist_write_view": {
            "field_description": ("Hide in blacklist when writing code view"),
            "help": "Hide from view when field is blacklisted.",
            "ttype": "boolean",
        },
        "is_show_whitelist_write_view": {
            "field_description": ("Show in whitelist when writing code view"),
            "help": (
                "If a field in model is in whitelist, all is not will be"
                " hide. "
            ),
