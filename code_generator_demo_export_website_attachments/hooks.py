...
"""
    ...
    value["exclude_dependencies_str"] = (
        "base;calendar;mail;muk_utils;muk_web_utils;web_editor"
    )
    ...
    # ir.attachment
    # Documentation from code odoo/addons/web_editor/controllers/main.py
    ir_attachment_model_id = env["ir.model"].search(
        [("model", "=", "ir.attachment")]
    )

    lst_ir_attachment_field_name = [
        "name",
        "type",
        "datas",
        "datas_fname",
    ...
    lst_ir_attachment_field = env["ir.model.fields"].search(
        [
            ("model_id", "=", ir_attachment_model_id.id),
            ("name", "in", lst_ir_attachment_field_name),
        ]
    )
    ...
    model_ids = (
        ir_ui_view_model_id
        + website_page_model_id
        + website_menu_model_id
        + ir_attachment_model_id
    )
    if not REPLACE_DEFAULT_WEBSITE:
